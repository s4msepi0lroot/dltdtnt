...
## Spotify заблокировал Web API без Premium — и что теперь

С февраля 2026 Spotify требует Premium **у того аккаунта, который владеет приложением** в Developer Dashboard. Если Premium нет, все запросы к Web API возвращают ошибку доступа:

> Your application is blocked from accessing the Web API since you do not have a Spotify Premium subscription.

Поэтому в моде есть второй источник данных, которому Web API вообще не нужен.

### Источники данных (Настройки → Аккаунт → «Источник данных»)

| Источник | Что делает | Требования |
| --- | --- | --- |
| `Авто` | Web API, если есть авторизация, иначе локальный источник Windows | — |
| `Spotify Web API` | Полный набор данных и управления | Premium у владельца приложения |
| `Windows (локально)` | Читает системную медиа-сессию Windows | Windows + запущенный десктопный Spotify |

### Как работает локальный режим

Мод читает `Windows.Media.Control.GlobalSystemMediaTransportControls` — ту же систему, что показывает плашку трека при нажатии клавиш громкости. Мост — небольшой PowerShell-скрипт `config/spotifysync-smtc.ps1`, который распаковывается из jar при первом запуске. Обложки кэшируются в `config/spotifysync-covers/`.

Доступно в локальном режиме:

- название, исполнитель, альбом;
- позиция и длительность трека (значит, синхронный текст песни работает как обычно);
- обложка альбома прямо из медиа-сессии;
- play / pause, следующий и предыдущий трек, перемотка.

Недоступно (ограничение системного API, а не мода):

- громкость, shuffle и repeat — кнопки просто ничего не делают;
- ссылка «открыть в Spotify», популярность трека и дата релиза.

Локальный режим работает с **бесплатным** аккаунтом Spotify, client id указывать не нужно. Работает и с другими плеерами (Яндекс Музыка, YouTube Music в браузере и т.д.) — достаточно изменить `localSessionFilter` в `config/spotifysync.json` (по умолчанию `spotify`) или очистить его, чтобы брать активную сессию.
